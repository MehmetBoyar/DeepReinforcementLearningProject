...
# Retrieves all experiment folders
folders = get_experiment_folders()

# We need two batches to perform a valid comparison
if len(folders) < 2:
    st.warning("Need at least 2 experiment folders to compare.")
else:
    # two columns Baseline vs. Candidate 
    c1, c2 = st.columns(2)
    with c1: 
    ...
    if st.button("Run Comparison", type="primary"):
        with st.spinner("Comparing..."):
            ...
            # The comparator generates CSVs and PNGs.
            run_comparison_suite(folder_a, folder_b, conf)
        

        # Display the generated dashboard image
        ...
        if os.path.exists("full_comparison_results.csv"):
            ...
            if "Baseline (Old)" in pivot and "Optimized (New)" in pivot:
                pivot["Improvement %"] = (
                    (pivot["Baseline (Old)"] - pivot["Optimized (New)"]) / pivot["Baseline (Old)"]
                ) * 100
                
                st.subheader("Improvement Statistics")
                
                # Positive improvement Green is good. Negative Red means regression.
                st.dataframe(
                    pivot.style.format({
                        "Baseline (Old)": "{:.2f}", 
                        "Optimized (New)": "{:.2f}", 
